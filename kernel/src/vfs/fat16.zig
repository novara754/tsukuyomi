...
fn readCluster(idx: u16, ebpb: *const EBPB, block_device: anytype, allocator: std.mem.Allocator) ![]align(@alignOf(DirEntry)) u8 {
    if (ebpb.bpb.bytes_per_sector != block_device.getBlockSize()) {
        panic("fat16.readFAT: block size mismatch", .{});
    }
...
pub fn findFile(path: []const u8, ebpb: *const EBPB, block_device: anytype, allocator: std.mem.Allocator) !DirEntry {
    if (path[0] != '/') {
    ...
    const fat = try readFAT(ebpb, block_device, allocator);
